...
// ExecReq sends a synchronous request and prints the pretty-printed response to stdout.
func ExecReq(nc *natsgo.Conn, subject string, payload []byte, timeoutMs int) error {
	if timeoutMs <= 0 {
		timeoutMs = 2000
	}
	...
	fmt.Println(prettyJSON(msg.Data))
	return nil
}

// ExecPub publishes a fire-and-forget message and prints confirmation to stdout.
func ExecPub(nc *natsgo.Conn, subject string, payload []byte) error {
...
// It calls js.Publish() synchronously and waits for the server Ack.
//
...
func ExecJS(nc *natsgo.Conn, entry *domain.TemplateEntry, payload []byte) error {
	js, err := nc.JetStream()
	if err != nil {
